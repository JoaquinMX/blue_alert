...
*/
  return (
    <Box>
        <GoogleMap center={center} zoom={10} mapContainerStyle={{width:'100%', height:'10rem'}}
            options={{
                zoomControl: false,
        ...
        >
            {myRef.pins.map((coordinates) => {
                const lat = coordinates[0];
                const lng = coordinates[1];
                const thePosition = { lat: lat, lng: lng};
                return <Marker position={thePosition} map={map} />
            })}
...
export default MapsOptimizedRoute;